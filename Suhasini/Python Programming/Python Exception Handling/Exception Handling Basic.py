...
# Raise explicit exception: When user wants to fail the program explicitly with the exception error
#                           --> This condition occurs when core application feature is not working. So it
#                               has to fix as soon as possible
...
def try_except_else_block_handling(num1, num2):
    try:
        ...
        print("Good Morning")
        # This block will execute only when there is no exception
...
# finally :  finally block of code will execute anyway, weather there is exception or not
...
######################################
# handle multiple exception: Can define different error msg for different exceptions
